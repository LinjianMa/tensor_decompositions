...
def CP_ALS(tenpy,
    ...
    if args is None:
        ...
        optimizer_list = {
            'DT':
            CP_DTALS_Optimizer(tenpy, T, A),
            'DTLR':
            CP_DTLRALS_Optimizer(tenpy, T, A, args),
            'PP':
            CP_PPALS_Optimizer(tenpy, T, A, args),
            'partialPP':
            CP_partialPPALS_Optimizer(tenpy, T, A, args),
            'NLS':
...
if __name__ == "__main__":
    ...
    if args.load_tensor is not '':
        ...
        if args.decomposition == "CP":
            ...
            sizes = [s] * args.order
            [T, O] = synthetic_tensors.init_rand(tenpy, order, sizes, R,
                                                 sp_frac, args.seed)
    ...
    if args.decomposition == "CP":
        if args.hosvd:
            ...
            CP_ALS(tenpy, A, compressed_T, O, 100, sp_res, csv_file, Regu,
                   'DT', args, args.res_calc_freq, nls_tol, cg_tol, grad_tol,
                   num, switch_tol, own_cg, nls_iter, als_iter, maxiter)
            A_fullsize = []
            for i in range(T.ndim):
                A_fullsize.append(tenpy.dot(transformer[i], A[i]))
            CP_ALS(tenpy, A_fullsize, T, O, num_iter, sp_res, csv_file, Regu,
                   args.method, args, args.res_calc_freq, nls_tol, cg_tol,
                   grad_tol, num, switch_tol, own_cg, nls_iter, als_iter,
                   maxiter)
        else:
            # TODO: it doesn't support sparse calculation with hosvd here
